...
export async function getIPv6Addr(stream: Duplex, service: NewService): Promise<void> {
  try {
    let remoteConfig: Ipv6ClientHandleTask;
    try {
      remoteConfig = JSON.parse(service.Config);
    } catch (err) {
      throw new Error(`解析 IPv6 配置失败: ${err}`);
    }
    clientTaskChan.send(remoteConfig);

    const ipv6Addr = getPublicIPv6();
    const ipv6Info: Ipv6ClientHandleTask = {
      RunId: '',
      Ipv6AddrIp: ipv6Addr,
